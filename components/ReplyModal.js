...
export default function ReplyModal ({id, handle, replyTo = null, quotes = null, showPostModal = null}) {
      ...
      <div class='flex w-screen place-content-center sm:h-screen pt-36 sm:p-0'>
        <div id='profile-preview' class='transition-opacity opacity-100 z-50 rounded-2xl sm:rounded-none shadow bg-white w-120 h-auto max-h-120 sm:max-h-screen overflow-auto sm:w-screen'>
          <div class=' border-b border-gray-300 p-3 sm:p-2'>
            <div onClick={()=>closeModal()} class='group content hover:bg-yellow-100 p-1 ml-1 sm:ml-3 rounded-full w-min cursor-pointer'>
              {
                (screen.width > 815)
                  ? <Icons name='x' options={{width: '7', color:'yellow-500', mdWidth: '7'}} /> 
                  : <Icons name='backArrow' options={{width: '7', color:'yellow-500', mdWidth: '7'}} /> 
              }
          ...
          <div class='p-2'>
            {replyTo && <Post id={replyTo} isInFeed={false}/>}
            <div class='pt-2'>
              <WritePost quotes={quotes} replyTo={replyTo} placeholder={'Post Your Reply'}/>
            </div>
          </div>
        </div>
      </div>
