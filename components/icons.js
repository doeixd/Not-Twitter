...
const svg = {
  repost: css => (
    <svg class={css***REMOVED*** viewBox="0 0 24 24">
      <g>
  ...
  commentFilled: css => (
    <svg class={css***REMOVED*** viewBox="-0.5 -0.5 7.5 7.5">
     <g>
      <path d="m2.1869 0c-1.2638 0-2.1869 1.0614-2.1869 2.3428 0 1.2814 0.92316 2.3428 2.1869 2.3428h0.75l-0.106 1.4735c0.0023 0.14163 0.15062 0.23304 0.27796 0.17129 0 0 0.89455-0.52992 1.6648-1.1856 0.77022-0.65563 1.5682-1.6105 1.5763-2.8007 1.9e-6 -4.318e-4 1.9e-6 -8.688e-4 0-0.0013 0-1.2917-1.0183-2.3428-2.2821-2.3428z" />
     </g>
    </svg>
  ),
  heart: css => (
...
export default function icons({name: initialIconName, options: initialOptions, changeOnPath = null, activeIconName = null***REMOVED***) {
  let defaultOptions = Object.assign(
      {height: 'auto', width: '5', color: 'gray-500', hoverColor: 'yellow-500', topOrBottom: 'bottom', mdWidth: '4.5', activeColor: null***REMOVED***,
      initialOptions
    )

  let { pathname, isReady, query ***REMOVED*** = useRouter()
  const isDoesChangeOnPathMatchQuery =
    query && Object.values(query).reduce((acc, cur) => {if (acc) return acc; return ('/' + cur) == changeOnPath***REMOVED***, false)

  if(changeOnPath && isReady){
    const isActive = (pathname == changeOnPath) || (isDoesChangeOnPathMatchQuery)

    const activeIconName =
      isActive
        ? (activeIconName ?? `${initialIconName***REMOVED***Filled`)
        : initialIconName

    const activeColor =
      isActive
        ? (defaultOptions?.activeColor ?? defaultOptions?.color)
        : defaultOptions.color

    var iconOptions = ({...defaultOptions, color: activeColor***REMOVED***)
    var iconName = activeIconName
***REMOVED***



  const {width, height, color, hoverColor, topOrBottom, mdWidth***REMOVED*** = iconOptions ?? defaultOptions

  const css = `w-${width***REMOVED*** h-${height***REMOVED*** sm:w-${mdWidth***REMOVED*** inline align-text-${topOrBottom***REMOVED*** text-${color***REMOVED*** group-hover:text-${hoverColor***REMOVED*** cursor-pointer stroke-1 fill-current`
  return svg[iconName ?? initialIconName](css)
***REMOVED***
