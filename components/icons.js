...
const svg = {
  repost: css => (
    <svg class={css} viewBox="0 0 24 24">
      <g>
  ...
  commentFilled: css => (
    <svg class={css} viewBox="-0.5 -0.5 7.5 7.5">
     <g>
  ...
  opts: css => (
    <svg viewBox="0 0 24 24" class={css}>
      <g>
...
export default function icons({name: initialIconName, options: initialOptions, changeOnPath = null, activeIconName = null}) {
  let defaultOptions = Object.assign(
      {height: 'auto', width: '5', color: 'gray-500', hoverColor: 'yellow-500', topOrBottom: 'bottom', mdWidth: '4.5', activeColor: null},
      initialOptions
    )

  let { pathname, isReady, query } = useRouter()
  const isDoesChangeOnPathMatchQuery =
    query && Object.values(query).reduce((acc, cur) => {if (acc) return acc; return ('/' + cur) == changeOnPath}, false)
  ...
  if(changeOnPath && isReady){
    ...
    const activeIconName =
      isActive
        ? (activeIconName ?? `${initialIconName}Filled`)
        : initialIconName
    ...
    var iconOptions = ({...defaultOptions, color: activeColor})
    var iconName = activeIconName
  }



  const {width, height, color, hoverColor, topOrBottom, mdWidth} = iconOptions ?? defaultOptions

  const css = `w-${width} h-${height} sm:w-${mdWidth} inline align-text-${topOrBottom} text-${color} group-hover:text-${hoverColor} cursor-pointer stroke-1 fill-current`
