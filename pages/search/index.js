...
import { getSearchResults ***REMOVED*** from '../../lib/queryHooks.js'

export default function TagPage({trends***REMOVED***) {
  ...
  return (<>
    ...
    { !trends
      ...
      : trends.map(trend => <Trend {...trend***REMOVED*** />)
  ***REMOVED***
  </>)

***REMOVED***

function Trend ({category, name, engagement***REMOVED***) {
  const router = useRouter()
...
***REMOVED***
  ...
  return (
    <div onClick={_ => search()***REMOVED*** class='p-3.5 space-y-0.5 hover:bg-gray-100 cursor-pointer'>
      <div class='text-sm text-gray-500'>{category***REMOVED***</div>
      <div class='font-bold text-gray-800'>{name***REMOVED***</div>
      {engagement && (<div class='text-sm text-gray-400'>{engagement***REMOVED***</div>)***REMOVED***
    </div>
  )
***REMOVED***

export async function getStaticProps() {
  const trends = await getTrendsFromTwitter() ?? {***REMOVED***
  return {
    props:{ trends ***REMOVED***,
    revalidate: 600
***REMOVED***
***REMOVED***
