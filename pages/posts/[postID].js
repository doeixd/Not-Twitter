...
export default function ProfilePage() {
  ...
  const {query: {postID***REMOVED*** ***REMOVED*** = router

  const id = useStore(state => state.id)
  const lastPage = useStore(state => state.lastPage)

  const {isLoading, error, data***REMOVED*** = getPost(postID)
  ...
  return (<>
    ...
    <div>
      <Post id={postID***REMOVED*** main={true***REMOVED*** />
    </div>
    {
     data.replies.map(p => <Post key={p***REMOVED*** id={p***REMOVED*** />) 
  ***REMOVED***
